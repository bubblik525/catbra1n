...
test('companion flow persists profile, provides free access, preserves paper results', async () => {
  ...
  assert.equal(app.snapshot().pet.xp, 0);
  await app.run({ type: 'investigate', address: a });
  assert.equal(app.snapshot().pet.xp, 0);
  ...
  await app.run({ type: 'watch', address: app.snapshot().tokens[5].address });
  assert.equal(Object.keys(app.snapshot().watch).length, 6);
  assert.equal(app.snapshot().access.name, 'Free');
