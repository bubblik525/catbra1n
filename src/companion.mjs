...
export function companion(state, save, options = {}) {
  ...
  async function checkAccess() { return; }
  const limits = () => ({name:'Free',tier:0,current:false,watch:100,rules:50,compare:8,fish:0});
  ...
  async function earn() { /* No progression in Brain Cat. */ }
  function snapshot() {
    return {
      demo: state.demo,
      connected: !!provider,
  ...
  async function action(body) {
    ...
    if (type === 'connect') {
      ...
      state.observedAddress = ''; // No identity or balance required.
      state.pet.name =
    ...
    } else if (['demo-tier','care','outfit'].includes(type)) {
      throw Error('Pet progression has been retired. All research tools are free.');
    } else if (type === 'watch') {
      ...
      if (state.watch[t.address]) delete state.watch[t.address];
        ...
        if (Object.keys(state.watch).length >= limits().watch)
          throw Error(
            `Workspace capacity: ${limits().watch} watched coins`,
          );
      ...
      if (state.trading.rules.length >= limits().rules)
        throw Error(`Workspace capacity: ${limits().rules} alerts`);
